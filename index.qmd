...
table_df["Absolute Change"] = table_df['2022'].astype(float) - table_df['2000'].astype(float)

# Calculate relative change as the absolute change divided by the 2000 emissions, then multiplied by 100
table_df["Relative Change"] = (table_df["Absolute Change"] / table_df['2000'].astype(float)) * 100
...
# Sort table_df alphabetically by country
table_df = table_df.sort_values(by='country').reset_index(drop=True)
```
...
emissions_long_subset = emissions_long_1990_2022.query("country in ['Singapore', 'China', 'India', 'Germany', 'Japan']")

# Create line chart. Year should be on the x-axis, emissions on the y-axis, and color should be by country.
fig_chart = px.line(emissions_long_subset, x="year", y="emissions", color="country", title="CO2 Emissions by Country (1990-2022)", labels={"year": "Year", "emissions": "CO2 Emissions", "country": "Country"})
